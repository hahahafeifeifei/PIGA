...
def rename_bed(bed, bed2, id_map_file):
    old2new = {}
    with open(id_map_file) as fr:
        for line in fr:
            a = line.strip().split()
...
def cal_supereads_overlap(fasta, outdir, bed, threads, min_ovlp_len, min_identity, o, r, super_ovlp_fast):
    '''
    ...
    if super_ovlp_fast:
        ...
        filter_distance = 'python ' + sys.path[0] + '/filter_overlap_by_distance.py ' + bed
        minimap = "minimap2 -cx ava-pb -Hk19 -Xw5 -m100 -g10000 --max-chain-skip 25 --end-bonus 100  " + \
                "-t %s %s %s |cut -f 1-12| %s | %s >%s" % (threads, fasta, fasta, filter_inline, filter_distance, paf)
...
def assemble_supereads(fasta, outdir, bed, threads, min_read_len, min_ovlp_len, min_identity, o, r,
                       max_tip_len, method, max_het_snps, min_allele_cov, type, rm_tmp, super_ovlp_fast):
    fasta2, paf, bed2 = None, None, None
    ...
    if method == 'rb':
        ...
        bed2 = outdir + '/supereads.align.rename.bed'
        rename_bed(bed, bed2, id_map_file)
        
        paf = cal_supereads_overlap(fasta2, outdir, bed2, threads, min_ovlp_len, min_identity, o, r, super_ovlp_fast)
    else:
        raise Exception('Warning: check the method for contig assembly.')

    digraph_file = ovlp2graph(fasta2, paf, rm_trans=1, threads=threads, remove_inclusions='false', rm_tips='true',

...
def assembly(args):
    ...
    for hap in ["1", "2"]:
        ...
        run_command("bwa mem -R '@RG\\tID:{}\\tSM:{}\\tPL:ILLUMINA' -t {} -Y {} {} {} | samtools view -uSh -@ {} | samtools sort -O bam -@ {} -o {}".format(args.prefix, args.prefix, args.threads, raw_contig_fa, os.path.join(ngs_polish_dir, "ngs.polish.R1.fastq"), os.path.join(ngs_polish_dir, "ngs.polish.R2.fastq"), args.threads, args.threads, os.path.join(ngs_polish_dir, "assembly.raw.contig.paired.bam")))
        run_command("bwa mem -R '@RG\\tID:{}\\tSM:{}\\tPL:ILLUMINA' -t {} -Y {} {} | samtools view -uSh -@ {} | samtools sort -O bam -@ {} -o {}".format(args.prefix, args.prefix, args.threads, raw_contig_fa, os.path.join(ngs_polish_dir, "ngs.polish.S.fastq"), args.threads, args.threads, os.path.join(ngs_polish_dir, "assembly.raw.contig.unpaired.bam")))

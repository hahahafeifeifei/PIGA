...
node_name_list = ["ID","Trans_ID","HET","HOM","AD","SN","Mini","Len","Homopolymer"] + ["Graphlet" + str(i) for i in range(0,73)] + ["training_y"]

node_input = sys.argv[1]
node_output_statistics = sys.argv[2]
node_output_dataset = sys.argv[3]
...
data = pd.read_csv(node_input, sep='\t', header=None, names=node_name_list, na_values=['NONE','nan'])
...
fo = open(node_output_statistics,"w")
feature_dict = {}
...
for feature in ["Graphlet" + str(i) for i in range(0,73)]:
...
#Normalization
for feature in ["HET","HOM","AD","SN"] + ["Graphlet" + str(i) for i in range(0,73)]:
...
#Create Dataset
X = torch.tensor(data[["HET","HOM","AD","SN","Mini","Len","Homopolymer"] + ["Graphlet" + str(i) for i in range(0,73)]].to_numpy(), dtype=torch.float32)
y = torch.tensor(data[['training_y']].stack().to_list(), dtype=torch.float32)
...
tensor_dataset = TensorDataset(X, y, train_mask, val_mask)
torch.save(tensor_dataset, node_output_dataset)

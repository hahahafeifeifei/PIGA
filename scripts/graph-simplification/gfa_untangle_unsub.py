...
def main():
    ...
    exten_size = 10
    max_region_size = int(sys.argv[4])
    jaccard_similarity = 0
    ref_include_sample_file = sys.argv[5]
    ref_candidate_sample_file = sys.argv[6]
    out_gfa_file = sys.argv[7]
    out_region_file = sys.argv[8]
    ...
    for query_region, query_info in region_info_dict.items():
        ...
        for target_region in target_region_candidate_set:
            ...
            if target_hap_name not in hap_jaccard_sim_dict or jaccard_sim > hap_jaccard_sim_dict[target_hap_name][1]:
                hap_jaccard_sim_dict[target_hap_name] = [target_region, jaccard_sim]
        ####Add the best target region of each target haplotype
        region_match_dict[query_region] = []
        for jaccard_info in hap_jaccard_sim_dict.values():
            if jaccard_info[1] > jaccard_similarity:
                region_match_dict[query_region].append(jaccard_info[0])
        processing += 1
        if processing % 100 == 0:
